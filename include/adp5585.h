...
extern struct udevice *adp5585_dev;

static int adp5585_i2c_write_reg(u8 reg, u8 val)
{
    int ret;

    ret = dm_i2c_write(adp5585_dev, reg, &val, 1);
    if (ret < 0){
        debug("dm_i2c_write error\n");
        return ret;
    }
    return 0;
}

static int adp5585_i2c_read_reg(u8 reg, u8 *val)
{
    int ret;

    ret = dm_i2c_read(adp5585_dev, reg, val, 1);
    if (ret < 0){
        debug("dm_i2c_read error\n");
        return ret;
    }
    return 0;
}
